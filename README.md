...
Основные элементы проекта были выполнены: сеть проверена, пользователь и группа созданы, права выставлены, nginx запускался, страница отдавалась через `curl`, capabilities для `tcpdump` проверены.

Главная ошибка — неполное и некорректное оформление SELinux-контекста в отчётных файлах. Для исправления необходимо применить постоянное правило `semanage fcontext`, выполнить `restorecon` и повторно приложить выводы `ls -Z`, `getenforce` и `curl`.
